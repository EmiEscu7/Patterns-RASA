...
class ContextManager():
    ...
    def __init__(self):
        ...
        self.dict_msg = {} #multitracker {"sender_id" : [msg_input, intent, utter/action]}
    ...
    def add_messege(self, sender_id, answer):
        """
        ...
        """
        if(sender_id == self.destiny):
            self.context_executor.there_is_interruption(sender_id)
        self.dict_msg[sender_id] = answer
    ...
    def decide_context(self) -> Dict:
        ...
        [rta, interruption] = self.context_executor.get_answer(self.dict_msg, self.destiny)
        # rta = [bot_destino, msj]. Lo mismo interruption
        if(interruption != None):
            self._set_destiny(interruption[0])
            return interruption[1]
        self._set_destiny(rta[0])
        return rta[1]
        ...
        **kwargs: Any,
        ...
        if(not self.answered):
            ...
            if(self.context_manager.give_sender()):
                ...
                if(sender_id != my_name):
                    sender_id = self.context_manager.change_sender()
                ...
                self.context_manager.change_give_sender()
                print("LA ANSWER FINAL ES: " + str(final_answer))
                rta = str(final_answer)                
            else:
